...
void volOrDraw(GEvent event, boolean bool) {
  if (event == GEvent.SELECTED) {
    if (bool) {
      volknob1.setVisible(true);
      backgroundNoDraw.setVisible(true);
      drawMode = false;
    } else {
      volknob1.setVisible(false);
      backgroundNoDraw.setVisible(false);
      drawMode = true;
    }
  }
}
...
void centerFileName(GTextField source, GEvent event) {
  if (event == GEvent.ENTERED) {
    ...
    String newText = "";
    int space = (int)(1.45*((len - revText.length())/2));
    for (int i = 0; i < space; i ++) {
      newText += " ";
    }
    trueFileName = revText;
    source.setText("");
    fileName.setPromptText(newText+revText);
  }
}
